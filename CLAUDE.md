...
- **Tasks.md** の該当タスクおよびサブタスクのチェックボックスを `- [x]` に更新する
- Backlog の該当課題の **description 内サブタスクのチェックボックス**を `- [x]` に更新する
...
## 7. エビデンス取得
- ワンショットで作成した一時ファイルの類は.logsフォルダに格納してください。
  - コンテンツをブラウザで確認する際にPlaywrightで取得したスクリーンショットなど。
...
## ドキュメント執筆ルール
- 当プロジェクトのドキュメントはMaterial for MkDocsで作成することを前提とする。
